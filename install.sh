...
# ------------------------------------------
# 9. Install Windsurf (AI IDE)
# ------------------------------------------
echo -e "Installing Windsurf (AI-Powered IDE)..."
if wget -q https://download.windsurf.ai/linux/latest.deb -O /tmp/windsurf.deb; then
    sudo dpkg -i /tmp/windsurf.deb || sudo apt -f install -y
    echo -e "Windsurf installed!"
else
    echo -e "Windsurf not available. Skipping."
fi
