...
read -p "Do you want to restore from backup? [y/N]: " restore_choice
restore_choice=${restore_choice,,}  # convert to lowercase
...
restore_app() {
    ...
    # Config files
    for f in settings.json keybindings.json shortcuts.json; do
        if [ -f "$src/$f" ]; then
            mkdir -p "$target"
            cp "$src/$f" "$target/"
            echo "   ➤ Restored $f"
    ...
    if [ -f "$src/extensions.tar.gz" ]; then
        ...
        tar -xzf "$src/extensions.tar.gz" -C "$target/extensions"
...
# --------------------------
if command -v ulauncher >/dev/null 2>&1; then
    restore_app "ulauncher" "$HOME/.config/ulauncher"
fi
...
# --------------------------
if command -v code >/dev/null 2>&1; then
    restore_app "vscode" "$HOME/.config/Code/User"
fi
...
# --------------------------
if command -v windsurf >/dev/null 2>&1; then
    restore_app "windsurf" "$HOME/.config/Windsurf/User"
fi

# --------------------------
# 4. Restore GNOME Extensions
# --------------------------
if [ -d "$LAST_BACKUP/gnome_extensions" ]; then
    mkdir -p "$HOME/.local/share/gnome-shell/extensions"
    tar -xzf "$LAST_BACKUP/gnome_extensions/extensions.tar.gz" \
