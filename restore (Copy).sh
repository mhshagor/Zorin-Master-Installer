...
if ! command -v ulauncher >/dev/null 2>&1; then
    ...
    ULAUNCHER_BACKUP_DIR="$MASTER_DIR/Backup/ulauncher"
    ULAUNCHER_CONF_DIR="$HOME/.config/ulauncher"
    ...
    if [ -d "$ULAUNCHER_BACKUP_DIR" ]; then
        echo "🔄 Restoring Ulauncher backup..."
        
        # Restore extensions
        if [ -f "$ULAUNCHER_BACKUP_DIR/extensions.tar.gz" ]; then
            mkdir -p "$ULAUNCHER_CONF_DIR/extensions"
            tar -xzf "$ULAUNCHER_BACKUP_DIR/extensions.tar.gz" -C "$ULAUNCHER_CONF_DIR/extensions"
            echo "✅ Extensions restored."
        fi

        # Restore settings
        mkdir -p "$ULAUNCHER_CONF_DIR"
        [ -f "$ULAUNCHER_BACKUP_DIR/settings.json" ] && cp "$ULAUNCHER_BACKUP_DIR/settings.json" "$ULAUNCHER_CONF_DIR/settings.json"
        [ -f "$ULAUNCHER_BACKUP_DIR/shortcuts.json" ] && cp "$ULAUNCHER_BACKUP_DIR/shortcuts.json" "$ULAUNCHER_CONF_DIR/shortcuts.json"
        echo "✅ Settings restored."
    else
        echo "📦 Installing default Ulauncher extensions..."
        mkdir -p "$ULAUNCHER_CONF_DIR/extensions"
        ...
        for name in "${!EXTENSIONS[@]}"; do
            ext_dir="$ULAUNCHER_CONF_DIR/extensions/$name"
            if [ ! -d "$ext_dir" ]; then
    ...
    echo "✅ uLauncher and extensions ready!"
else
