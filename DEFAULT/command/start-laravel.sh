...
if [ -z "$APACHE_RUNNING" ]; then
    ...
    echo ""
    echo "🚀 Launching XAMPP..."
    sudo /opt/lampp/lampp start
    echo "✅ XAMPP started successfully!"
else
    echo "✅ XAMPP already running, skipping..."
fi
...
if [ ! -d "$PROJECT_PATH" ]; then
    echo "❌ Current directory '$PROJECT_PATH' does not exist!"
    exit 1
fi
...
if ! grep -q "$HOST_NAME" /etc/hosts; then
    ...
    echo "✅ Host '$HOST_NAME' already exists in /etc/hosts."
fi
...
# -------------------------------
echo ""
echo "🟢 Preparing to start Laravel server..."
...
# -------------------------------
echo ""
echo "🕔 Laravel server will start in 5 seconds..."
for i in {5..1}; do
    echo -ne "➡️  Starting in $i...\r"
    sleep 1
done
echo ""
echo "🚀 Starting Laravel server at http://$HOST_NAME:$PORT"
echo ""
